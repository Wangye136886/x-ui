...
install() {
    bash <(curl -Ls https://gitlab.com/ye857257/x-ui/master/install/x-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://gitlab.com/ye857257/x-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        LOGI "更新完成，已自动重启面板 "
        exit 0
    fi
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/Wangye136886/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
